...
while True:
    ...
    for event, _ in events:
        ...
        ):  # if event target socket is server_socket then event is connection attempt
            connection, address = server_socket.accept()
            connection.setblocking(False)
            print(f"Connection request from {address} received")
            selector.register(
                connection, selectors.EVENT_READ
            )  # register new client socket in selector
        else:
            data = event_socket.recv(
                1024
            )  # if event target socket is client socket - get data from it and send it back
